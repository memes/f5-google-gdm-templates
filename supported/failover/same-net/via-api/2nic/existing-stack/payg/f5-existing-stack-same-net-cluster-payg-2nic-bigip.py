...
def Metadata(context,group, storageName, licenseType):
  ...
  if ALLOWUSAGEANALYTICS in ['yes', 'true']:
      ...
      SENDANALYTICS = ' --metrics "cloudName:google,region:' + context.properties['region'] + ',bigipVersion:' + context.properties['imageName'] + ',customerId:${CUSTOMERID},deploymentId:${DEPLOYMENTID},templateName:f5-existing-stack-same-net-cluster-payg-2nic-bigip.py,templateVersion:3.12.0,licenseType:payg"'
  else:
  ...
  metadata = {
              'items': [{
                  ...
                  'value': ('\n'.join(['#!/bin/bash',
                                    ...
                                    'declare -a filesToVerify=(\"/config/cloud/f5-cloud-libs.tar.gz\" \"/config/cloud/f5-cloud-libs-gce.tar.gz\" \"/var/config/rest/downloads/f5-appsvcs-3.26.1-1.noarch.rpm\" \"/var/config/rest/downloads/f5-cloud-failover-1.8.0-0.noarch.rpm\")',
                                    ...
                                    '/usr/bin/curl -s -f -L --retry 20 -o /var/config/rest/downloads/f5-cloud-failover-1.8.0-0.noarch.rpm https://github.com/F5Networks/f5-cloud-failover-extension/releases/download/v1.8.0/f5-cloud-failover-1.8.0-0.noarch.rpm',
